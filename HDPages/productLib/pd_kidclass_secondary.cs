...
namespace HDPages.productLib
{
    public class pd_kidclass_secondary : baseBusiness
    {
        ...
        public DataTable getEditdatabyID(string strID, string name)
        {
            string sqltext = " select sm.ID,sm.mainID,sm.classname,sm.orderint,sm.darycode,sm.adddate " +
                " from pd_kidclass_secondary sm " +
                " where sm.classname='" + name + "' and sm.mainID='" + strID + "'";
        ...
        public DataTable getmachinekit(string strmtypename)
        {
            ...
            {
                strmtypename = " where mainID in (select ID from pd_kidclass_main where mainname='" + strmtypename + "') ";
            }
            else
            {
                strmtypename = "";
            ...
            string sqltext = "select classname " +
                    " from pd_kidclass_secondary " + strmtypename;
