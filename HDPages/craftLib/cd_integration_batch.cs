...
{
   public class cd_integration_batch:baseBusiness
    {
        #region page
        //分页get_node
        ...
        public DataTable getBindDataAsdt(Dictionary<string, string> dic, string strsort, string strSortDirection, int intPagesize, int intPageindex)
        {
            string sqltext = " select * from (select ROW_NUMBER() OVER(Order by bh." + strsort.Trim() + " " + strSortDirection + ") as rowno," +
                " * " +
                " from cd_integration_batch bh " +
                " where bh.iid = '" + dic["iid"].ToString().Trim() + "' ) as s " +
                " where rowno >" + ((intPageindex - 1) * intPagesize).ToString() + " and rowno<=" + (intPageindex * intPagesize).ToString() + " ";
        ...
        public DataTable getEditdata(string strID)
        {
            ...
            "      ,[systemdate]												   " +
            "      ,[isdelid]													   " +
            "  FROM [HDPMWDB].[dbo].[cd_integration_batch]								   " +
        ...
        {
            string sqltext = "INSERT INTO[dbo].[cd_integration_batch]"
             + "        ([ID]                           "
             + "       ,[iid]                           "
             + "       ,[iname]                         "
             + "       ,[batchnumber]                   "
             ...
             + "       ,[systemdate]                    "
             + "       ,[isdelid]                       "
             + "    FROM[dbo].[cd_integration_batch]     "
             + "    where iid = '" + mechanicalcard_from + "'  )    ";

            return DBSQL.ExecutesqlTobool(sqltext, DBSQL.connstr) > 0;
        }
        public string[] getnewIDlist(string newcard)
        {
            string sqltext = "select ID from cd_integration_batch where iid = '" + newcard + "'";
        ...
        {
            string sqltext = "select ID from cd_integration_batch where iid = '" + fromcard + "'";
