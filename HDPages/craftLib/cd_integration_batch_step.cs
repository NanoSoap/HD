...
{
    public class cd_integration_batch_step:baseBusiness
    {
        //分页get
        public DataTable getBindDataAsdt(Dictionary<string, string> dic, string strsort, string strSortDirection, int intPagesize, int intPageindex)
        {
            string sqltext = " select * from (select ROW_NUMBER() OVER(Order by step." + strsort.Trim() + " " + strSortDirection + ") as rowno," +
                " * " +
                " from cd_integration_batch_step step " +
        ...
        {
            return "INSERT INTO [dbo].[cd_integration_batch_step]	 " +
                    "           ([ID]								 " +
                    "           ,[batchid]							 " +
                    "           ,[snumber]							 " +
                    "           ,[stext]							 " +
                    ...
                    "			  ,[systemdate]						 " +
                    "			  ,[isdelid]						 " +
                    "		  FROM [dbo].[cd_integration_batch_step] where batchid  ='" + batch_from + "'	 ";
        }
    }
}
