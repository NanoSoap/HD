...
{
    public partial class mechanicalDesignManage : PageBase
    {
        ...
        private void initbaseinfo(string strID)
        {
            if (strID.Trim() != "")
            {
                ...
                DataTable dt = pm.getEditdata_mechanical(strID);

                if (dt != null && dt.Rows.Count == 1)
                {
                    DataRow r = dt.Rows[0];
                    kitname.Text = r["mname"].ToString().Trim();
                    kitcode.Text = r["bcode"].ToString().Trim();
        ...
        private void inittree()
        {
            ...
            // 模拟从数据库返回数据表
            pd_machinekit pm = new pd_machinekit();
            DataTable table = pm.getBindTreeDataAsdtClone();
            ...
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                if (row.IsNull("pid"))
                {
                    ...
                    node.Attributes.Add("hascard", row["hascard"].ToString().Trim());
        ...
        private void BindGrid()
        {
            ...
            string stroname = Fmname.Text.Trim();

            Dictionary<string, string> dic = new Dictionary<string, string>();

            dic.Add("mname", stroname);
            dic.Add("kitname", Fkitname.Text.Trim());
            ...
            DataTable dt = mc.getBindDataAsdt(dic, strSort, strSortDirection, intPagesize, intPageindex);
            DataTable dt1 = mc.getBindDataAsdt(dic, strSort, strSortDirection);
        ...
        private void BindGridNode()
        {
            ...
            DataTable dt = mc.getBindDataAsdtNode(dic, strSort, strSortDirection, intPagesize, intPageindex);
            DataTable dt1 = mc.getBindDataAsdtNode(dic, strSort, strSortDirection);
            TotalPage.Text = dt1.Rows.Count.ToString();
        ...
        protected void btnNew_Click(object sender, EventArgs e)
        {
            ...
            if (firsttree.SelectedNode!=null&&firsttree.SelectedNode.Attributes.GetValue("nodemenu").ToString()=="2")
            {
                if (!new cd_mechanical().existCard(firsttree.SelectedNode.NodeID))
                {
                    erase();
        ...
        {
            cd_mechanical mc = new cd_mechanical();
            DataTable dt = mc.getEditdata(strID);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (intRowindexarray.Length > 0)
            {
                ...
                int intresult = mc.deletebycondition("cd_mechanical", dic);
...

        {
            ...
            Dictionary<string, string> dicMechanicalDesign = initDatadicmechanical();
            ...
            if (strID == "")
            {
                ...
                dicMechanicalDesign.Add("ID", mechanicalDesignID);
                dicMechanicalDesign.Add("isdelid", "1");
                dicMechanicalDesign.Add("pid", firsttree.SelectedNode.NodeID);
                //dicMechanicalDesign.Add("mid", Grid1.DataKeys[Grid2.SelectedRowIndexArray[0]][0].ToString());
                string str_mname = dicMechanicalDesign["mname"].ToString().Trim();
                string str_mcode = dicMechanicalDesign["mcode"].ToString().Trim();
                if ((mc.isExistdata("cd_mechanical", "mname", str_mname, "*").Trim() != "") && (mc.isExistdata("cd_mechanical", "mcode", str_mcode, "*").Trim() != ""))
                {
                    Alert.Show(" 该名称或编号已经存在!");
                }
                else
                {
                    intresultRecord = mc.add(dicMechanicalDesign, "cd_mechanical");
                    setPageContent(1);
                }
            }
            else
            {
                intresultRecord = mc.update(dicMechanicalDesign, "cd_mechanical", "ID", strID);
        ...
        private Dictionary<string, string> initDatadicmechanical()
        {
            Dictionary<string, string> dic = new Dictionary<string, string>();
            dic.Add("mname", mname.Text.ToString().Trim());
            dic.Add("kitname", kitname.Text);
            dic.Add("kitcode", kitcode.Text);
            dic.Add("mcode", mcode.Text);
            dic.Add("mtag", mtag.Text);
        ...
        protected void toolsave_Click(object sender, EventArgs e)
        {
            ...
            foreach (int rowIndex in modifiedDict.Keys)
            {
                ...
                update += bh.update(objToString(dic), "cd_mechanical_batch", "ID", rowID);
            }
            ...
            foreach (int rowIndex in deletedRows)
            {
                ...
                string rowID = Grid2.DataKeys[rowIndex][0].ToString();
                //bh.delete("cd_mechanical_batch","ID",rowID);

                //bh.delete("cd_mechanical_batch_step", "batchid", rowID);
                sqls[0] = createSql.DeleteDatasql("cd_mechanical_batch", "ID", rowID);
                sqls[1] = createSql.DeleteDatasql("cd_mechanical_batch_step", "batchid", rowID);
            ...
            foreach (var item in newAddedList)
            {
                ...
                dic.Add("mid", Grid1.DataKeys[Grid1.SelectedRowIndexArray[0]][0].ToString());
                dic.Add("mname", Grid1.DataKeys[Grid1.SelectedRowIndexArray[0]][1].ToString());
                dic.Add("operater", SessionUserName);
                dic.Add("systemdate", DateTime.Now.AddMilliseconds(i * 1000).ToString());
                dic.Add("isdelid", "1");
                news += bh.add(dic, "cd_mechanical_batch");
                i++;
            }
        ...
        protected void toolSaveStep_Click(object sender, EventArgs e)
        {
            ...
            foreach (int rowIndex in modifiedDict.Keys)
            {
                ...
                update += step.update(objToString(dic), "cd_mechanical_batch_step", "ID", rowID);
            }
            ...
            {
                string rowID = Grid3.DataKeys[rowIndex][0].ToString();
                delete += step.delete("cd_mechanical_batch_step", "ID", rowID);
            }
            ...
            foreach (var item in newAddedList)
            {
                ...
                dic.Add("systemdate", DateTime.Now.AddMilliseconds(i * 1000).ToString());
                dic.Add("isdelid", "1");
                news += step.add(dic, "cd_mechanical_batch_step");
                i++;
            }
        ...
        //        DataRow r = dt.Rows[0];
        //        //
        //        mname.Text = r["mname"].ToString() + "（克隆）";
        //        mtag.Text = r["mtag"].ToString();
        //        rawtype.Text = r["rawtype"].ToString();
        //        rawsize.Text = r["rawsize"].ToString();
        //        nperraw.Text = r["nperraw"].ToString();
        //        nperdesk.Text = r["nperdesk"].ToString();
        //        //mcode.Text = r["mcode"].ToString() + "（克隆）";
        //        //
        //        string ID_mechanicalCard = Guid.NewGuid().ToString();
        //        Dictionary<string, string> dic_mechanical = initDatadicmechanical();
        //        dic_mechanical.Add("ID", ID_mechanicalCard);
        //        dic_mechanical.Add("pid", firsttree.SelectedNode.NodeID);
        //        mc.add(dic_mechanical, "cd_mechanical");
        ...
        private string getCardCode()
        {
            ...
            return new xparams().getparamData("H015").Split(',')[4].Split('|')[1] + Rand.Number(8);
        }
        ...
        {
            cd_mechanical mc = new cd_mechanical();
            strCloneID.Text = firsttree.SelectedNode.NodeID;
            DataTable dt = mc.getClonedata(strCloneID.Text);
        ...
        {
            DataTable dt0 = new cd_mechanical().getClonedata(firsttree.SelectedNode.NodeID);
